...
PARLIANT's remote MCP boundary is **disabled by default**. The implementation never turns the daemon into a public listener and refuses non-loopback bind addresses.

The local endpoint speaks MCP `2026-07-28` over stateless HTTP at `POST /mcp`. It implements `server/discover` and exposes only the bounded read-only meeting tools already provided by `parliant-mcp`:
...
Runtime certification still requires an actual Secure MCP Tunnel plus a supported ChatGPT/OpenAI client connection. That test must confirm the cloud client can call the read-only tools while PARLIANT remains unreachable directly from the public network, then confirm revocation breaks subsequent access.
