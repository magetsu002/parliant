...
M5 exposes retained meeting context through a local, read-only MCP JSON-RPC service. The implemented tools are `meeting_get_recent`, `meeting_search`, `meeting_get_segment`, `meeting_get_speakers`, and `meeting_get_status`. No summary tool is advertised because SIDECAR does not yet maintain summary state.
