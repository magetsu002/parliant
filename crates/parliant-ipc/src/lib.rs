//! Versioned, bounded local IPC between the PARLIANT daemon and presentation-only overlay.
...
pub enum IpcError {
    ...
    UnsafeSocketPath,
    #[error("IPC peer uid does not match the PARLIANT process uid")]
    PeerUidMismatch,
...
pub fn default_socket_path() -> Result<PathBuf, IpcError> {
    ...
    Ok(PathBuf::from(runtime).join("parliant").join("overlay.sock"))
}
...
impl IpcServer {
    ...
    pub fn spawn(self) -> Result<IpcServerHandle, IpcError> {
        ...
        let accept_thread = thread::Builder::new()
            .name("parliant-overlay-ipc".to_string())
            .spawn(move || {
                    ...
                    match listener.accept() {
                        Ok((stream, _)) => {
                            ...
                            let _ = thread::Builder::new()
                                .name(format!("parliant-overlay-client-{client_id}"))
                                .spawn(move || {
                                    let _ = handle_client(
                                        client_id,
                                        stream,
...
fn handle_client(
    ...
    let writer = thread::Builder::new()
        .name(format!("parliant-overlay-writer-{client_id}"))
        .spawn(move || {
