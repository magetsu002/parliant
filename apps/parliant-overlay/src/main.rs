...
#[derive(Debug, Parser)]
#[command(
    name = "parliant-overlay",
    about = "Private local PARLIANT meeting overlay"
)]
struct Cli {
...
fn main() {
    ...
    let socket = match cli.socket.map(Ok).unwrap_or_else(default_socket_path) {
        Ok(path) => path,
        Err(error) => {
            eprintln!("parliant-overlay: {error}");
            return;
        }
    };

    let application = Application::builder()
        .application_id("dev.magetsu.parliant.overlay")
        .build();
...
fn build_ui(application: &Application, socket: PathBuf) {
    ...
    window.set_namespace("parliant-overlay");
    ...
    let status = Label::new(Some("Connecting to PARLIANT…"));
...
fn spawn_ipc_worker(
    ...
    let _ = thread::Builder::new()
        .name("parliant-overlay-ipc-client".to_string())
        .spawn(move || loop {
